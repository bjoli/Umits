...
public partial class SettingsPage : ContentPage
{
    ...
    protected override async void OnAppearing()
    {
        ...
        {
            MacrosEditor.Text = await File.ReadAllTextAsync(_macrosFilePath);
        }

        // Load Entities
        if (File.Exists(_entitiesFilePath))
        {
            EntitiesEditor.Text = await File.ReadAllTextAsync(_entitiesFilePath);
        }
    }
    ...
    private void OnEditorFocused(object sender, FocusEventArgs e)
    {
        ...
        {
            editor.Animate("Expand", new Animation(v => editor.HeightRequest = v, editor.Height, ExpandedHeight), 16, 250, Easing.CubicOut);
        }
    }
    ...
    private void OnEditorUnfocused(object sender, FocusEventArgs e)
    {
        if (sender is Editor editor)
        {
            editor.Animate("Collapse", new Animation(v => editor.HeightRequest = v, editor.Height, CollapsedHeight), 16, 250, Easing.CubicIn);
        }
    }
